...
# Make Dataset
t = np.log(rdf[outcome].reset_index().drop('index', axis=1))
...
for i in range(100):  # for 100 repeates
    ...
    for train_index, test_index in try_cv.split(X):
        ...
        # log transform
        unscaled_weights = funcs.log_transform_weights(un_scaled_weights)
...
#             bbox=dict(boxstyle='round', fc='w'),
#             size=8, ha='left', va='top')
fig.savefig("D:\\Etienne\\fall2022\\agu_data\\results\\scaled_X_LOG\\all_sites_scaledX_nolog_cv.png", dpi=500,
            bbox_inches='tight')
plt.show()
...
# for key in hold_marsh_weights:
#     fig, ax = plt.subplots()
#     ax.bar(hold_marsh_weights[key], color='yellow', align='center')
#     ax2 = ax.twinx()
#     ax2.boxplot(hold_unscaled_weights)
...
funcs.wrap_labels(ax, 10)
fig.savefig("D:\\Etienne\\fall2022\\agu_data\\results\\scaled_X_LOG\\regularization_scaledX_nolog_boxplot.png",
            dpi=500,
            bbox_inches='tight')
plt.show()
...
funcs.wrap_labels(ax, 10)
fig.savefig("D:\\Etienne\\fall2022\\agu_data\\results\\scaled_X_LOG\\certainty_scaledX_nolog_boxplot.png",
            dpi=500,
            bbox_inches='tight')
plt.show()
...
funcs.wrap_labels(ax, 10)
fig.savefig("D:\\Etienne\\fall2022\\agu_data\\results\\scaled_X_LOG\\intercepts_nolog_boxplot.png", dpi=500, bbox_inches='tight')
plt.show()
...
funcs.wrap_labels(ax, 10)
fig.savefig("D:\\Etienne\\fall2022\\agu_data\\results\\scaled_X_LOG\\pred_certainty_scaledX_nolog_boxplot.png",
            dpi=500,
            bbox_inches='tight')
plt.show()
